...
class My_dataloader(Dataset):
	...
	def __init__(self, data_24, transform):
		...
		self.pathes_24 = glob(self.data_24+'/**/*.png',recursive = True)
...
def test_model(model, criterion, num_epochs=25):
	...
	for i in range(1):
		...
		test_path_test = '/mnt/largedrive0/katariap/feature_extraction/data/Dataset/Images_Tiled'
		test_imagedataset = My_dataloader(test_path_test,trans)
		print(len(test_imagedataset))
		dataloader_test = torch.utils.data.DataLoader(test_imagedataset, batch_size=16,shuffle=False, num_workers=1)
		counter = 0
		# Iterate over data.
...
model_final.load_state_dict(torch.load('/mnt/largedrive0/katariap/feature_extraction/data/Code/kimianet_weights/KimiaNetPyTorchWeights.pth'))
